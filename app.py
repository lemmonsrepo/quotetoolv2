...
if not st.session_state.submitted:
    ...
    for row in rows:
        ...
        for i, key in enumerate(row):
            ...
            if st.session_state.age_input and len(st.session_state.age_input) == 1 and int(st.session_state.age_input + key) > 80:
                disable = True
            if st.session_state.age_input == "" and key == "9":
                disable = True
            with cols[i]:
    ...
    cols = st.columns(3)
    with cols[0]:
        st.button("MALE", on_click=submit, args=("Male",), disabled=len(st.session_state.age_input) != 2, key="male")
    with cols[1]:
        st.button("0", on_click=add_digit, args=("0",), disabled=st.session_state.age_input != "8", key="btn0")
    with cols[2]:
        st.button("FEMALE", on_click=submit, args=("Female",), disabled=len(st.session_state.age_input) != 2, key="female")
...
if st.session_state.submitted and st.session_state.age_input.isdigit():
    ...
    st.markdown("""
        <style>
        .results { font-family: Myriad Pro; font-weight: bold; font-size: 22px; text-align: center; }
        </style>
    """, unsafe_allow_html=True)

    st.markdown(f"<div class='results'>({age}{g_abbr})</div>", unsafe_allow_html=True)

    if age >= 65:
        fe = final_expense_prices[age][gender]
        out = f"FE ${fe}"
        st.session_state.copy_text = f"({age}{g_abbr})\n{out}"
        st.markdown(f"<div class='results'>{out}</div>", unsafe_allow_html=True)
    else:
        if age <= 45:
            plan = "IA"
    ...
    # Actual clipboard copy with JS
    st.markdown(f"""
        <textarea id="copybox" style="opacity:0; height:1px">{st.session_state.copy_text}</textarea>
        <button onclick="navigator.clipboard.writeText(document.getElementById('copybox').value); alert('Copied!');">COPY</button>
    """, unsafe_allow_html=True)
