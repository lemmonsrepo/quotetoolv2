...
if st.session_state.submitted and st.session_state.age_input.isdigit():
    ...
    if age >= 65:
        ...
        st.markdown(f"<div class='results bold'>FE ${fe}</div>", unsafe_allow_html=True)
    else:
        ...
        bundle = price + sh
        st.session_state.copy_text = f"({age}{g_abbr})\n{plan}${price} | SH${sh}\nBUNDLE ${bundle}"
        st.markdown(f"<div class='results'><span class='bold'>{plan}${price}</span> <span style='font-weight: normal;'>|</span> <span class='bold'>SH${sh}</span></div>", unsafe_allow_html=True)
        st.markdown(f"<div class='results'><span class='bold'>BUNDLE ${bundle}</span></div>", unsafe_allow_html=True)
