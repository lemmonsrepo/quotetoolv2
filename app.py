...
if st.session_state.submitted and st.session_state.age_input.isdigit():
    ...
    if plan == "FE":
        ...
        html_block = f"""
        <div onclick="navigator.clipboard.writeText(`{copy_block}`)" style='cursor: pointer; font-family: Myriad Pro; color: white; font-size: 22px; text-align: center; line-height: 1.6; background-color: #2c2c2c; padding: 10px; border-radius: 8px; margin-top: 20px;'>
            ({age}{g_abbr})<br>
            <b>FE ${price}</b>
        </div>
        """
